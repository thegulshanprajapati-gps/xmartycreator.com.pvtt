  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 text-slate-900">
      <div className="relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute -left-24 top-12 h-64 w-64 rounded-full bg-blue-300/20 blur-3xl" />
        <div className="pointer-events-none absolute right-0 top-8 h-72 w-72 rounded-full bg-purple-300/16 blur-3xl" />
        <div className="pointer-events-none absolute left-1/3 bottom-6 h-56 w-56 rounded-full bg-cyan-300/16 blur-3xl" />
        <header className="container mx-auto px-4 md:px-6 max-w-6xl pt-12 pb-10 relative space-y-8">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500/12 via-purple-500/12 to-pink-500/12 border border-slate-200/60 text-slate-800 rounded-full px-4 py-2 w-fit shadow-[0_10px_40px_-30px_rgba(59,130,246,0.8)] transition-colors">
          ...
          <div className="flex flex-wrap gap-8 items-start">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                {hero.secondaryButton?.text && (
                  <Button
                    ...
                    size="lg"
                    variant="outline"
                    className="border border-slate-300/90 text-slate-900 hover:bg-slate-100/90"
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full md:w-auto md:min-w-[320px]">
              {hero.pills.slice(0,2).map((pill:any, i:number) => (
                <div key={i} className="rounded-2xl border border-slate-200/70 bg-white/85 px-4 py-4 shadow-[0_15px_50px_-35px_rgba(0,0,0,0.25)] backdrop-blur">
                  <div className="flex items-center gap-3 mb-2 text-slate-700">
                    {i === 0 ? <BookOpen className="w-5 h-5" /> : <PenLine className="w-5 h-5" />}
                    <span className="text-sm font-semibold text-slate-900">{pill.title || (i===0?'Deep dives':'Creator desk')}</span>
                  </div>
                  <div className="text-sm text-slate-700">{pill.description || (i===0?'Guides & how-tos':'Release briefs')}</div>
                </div>
              ))}
            </div>
          </div>
